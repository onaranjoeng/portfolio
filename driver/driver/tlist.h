...
public:
    ...
    void swap(TList& x, TList& y)  // helper function for the move and copy constructor/operator
    {
        TList<T> tmp;
        tmp = x;
        x = y;
...
TList<T> operator+(const TList<T>& t1, const TList<T>& t2) {
    ...
    TList<T> result = t1;                        // copying the first list

    if (t2.GetSize() == 0) return result;        //if t2 is empty, 
